...
const LEGACY_RECORDING_STATE_OVERRIDE_ENV = "QA_VOICE_RECORDING_STATE_PATH";
...
export function recordingStateFilePath(
  ...
  return readOverride(
    [RECORDING_STATE_OVERRIDE_ENV, LEGACY_RECORDING_STATE_OVERRIDE_ENV],
    join(
      STATE_DIR,
      isDevInstance(env) ? "recording-state-dev.json" : "recording-state.json",
    ),
    env,
  );
}

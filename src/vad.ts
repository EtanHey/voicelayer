...
function findModelPath(): string {
  ...
  const cacheModel = join(process.env.HOME || "~", ".cache", "voicelayer", "silero_vad.onnx");
...
async function getVADSession(): Promise<VADSession> {
  ...
  cachedSession = { session, state, sr };
...
export async function processVADChunk(pcmChunk: Uint8Array): Promise<number> {
  ...
  const floats = new Float32Array(VAD_CHUNK_SAMPLES);
  for (let i = 0; i < VAD_CHUNK_SAMPLES; i++) {
    floats[i] = view.getInt16(i * 2, true) / 32768.0;
  }

  const input = new ort.Tensor("float32", floats, [1, VAD_CHUNK_SAMPLES]);
 ...
 * Clears the RNN hidden state so next recording starts fresh.
 */

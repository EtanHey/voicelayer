...
describe("ack protocol", () => {
  ...
  let androidResultSpy: ReturnType<typeof spyOn>;
  ...
  beforeEach(() => {
    ...
    androidPullSpy = spyOn(
      androidBridge.AndroidApplianceBridge.prototype,
      "pullAudio",
    ).mockResolvedValue({ ok: true, bytes: new Uint8Array([82, 73, 70, 70]) });
    androidResultSpy = spyOn(
      androidBridge.AndroidApplianceBridge.prototype,
      "getResult",
    ).mockResolvedValue({
      ok: true,
      ...
      durationMs: 1200,
      audioAvailable: true,
      transcript: { text: "phone transcript", partial: false },
      events: [],
    });
  ...
  it("rejects Android recording visibly when the appliance is unavailable", async () => {
    queueDepthSpy.mockReturnValue(0);
    recordingStateSpy.mockReturnValue("idle");
    ...
    expect(broadcastSpy).toHaveBeenCalledWith({
      type: "error",
      message: "Android VoiceLayer unavailable: busy",
      recoverable: true,
  ...
  it("stops an active Android recording and hands audio to Mac transcription", async () => {
    ...
    expect(androidResultSpy).toHaveBeenCalledWith("android-session", 0);
    ...
    expect(transcribeExternalSpy).toHaveBeenCalledWith(
      new Uint8Array([82, 73, 70, 70]),
      {
        ...
        pressToTalk: true,
        applianceTranscript: "phone transcript",
        androidSessionId: "android-session",
      },
    );
    expect(response).toEqual({
      type: "ack",
  ...
  it("pulls and transcribes Android audio when the phone returns no transcript", async () => {
    queueDepthSpy.mockReturnValue(0);
    recordingStateSpy.mockReturnValue("idle");
    process.env.QA_VOICE_ANDROID_APPLIANCE = "1";
    androidStartSpy.mockResolvedValueOnce({
      ok: true,
      sessionId: "android-audio-only",
      source: "android",
    });
    androidStopSpy.mockResolvedValueOnce({
      ok: true,
      durationMs: 1200,
      transcript: null,
      audioSha256: "sha",
    });
    androidResultSpy.mockResolvedValueOnce({
      ok: true,
      sessionId: "android-audio-only",
      state: "ready",
      seq: 2,
    ...
    const response = handleSocketCommand({
      cmd: "stop",
      id: "stop-android-audio-only",
    } as unknown as SocketCommand);
    await new Promise((resolve) => setTimeout(resolve, 0));

    expect(androidPullSpy).toHaveBeenCalledWith("android-audio-only");
    expect(transcribeExternalSpy).toHaveBeenCalledWith(
      new Uint8Array([82, 73, 70, 70]),
      {
        silenceMode: "standard",
        pressToTalk: true,
        applianceTranscript: null,
        androidSessionId: "android-audio-only",
      },
    );
    expect(response).toEqual({
      type: "ack",
  ...
  it("forwards Android partial transcript events while stopping", async () => {
    queueDepthSpy.mockReturnValue(0);
    recordingStateSpy.mockReturnValue("idle");
    process.env.QA_VOICE_ANDROID_APPLIANCE = "1";
    androidStartSpy.mockResolvedValueOnce({
      ok: true,
      sessionId: "android-session-events",
      source: "android",
    });
    androidResultSpy.mockResolvedValueOnce({
      ok: true,
      ...
      durationMs: 1200,
      audioAvailable: true,
      transcript: { text: "phone final", partial: false },
      events: [
        {
          seq: 2,
          type: "transcription",
    ...
    recordingStateSpy.mockReturnValue("recording");
    handleSocketCommand({ cmd: "stop", id: "stop-events" } as unknown as SocketCommand);
    await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    expect(broadcastSpy).toHaveBeenCalledWith({
      ...
      partial: true,
      source: "android",
      session_id: "android-session-events",
    });
    expect(transcribeExternalSpy).toHaveBeenCalledWith(
      new Uint8Array([82, 73, 70, 70]),
      expect.objectContaining({
        applianceTranscript: "phone final",
        androidSessionId: "android-session-events",
      }),
    );
  });

...
describe("AndroidApplianceBridge", () => {
  ...
  it("treats busy start responses as an Android-mode failure", async () => {
    serve((req) => {
      const url = new URL(req.url);
      if (url.pathname === "/health") {
        return Response.json({ ok: true, api_version: 1, state: "idle" });
  ...
  it("stops a recording and pulls WAV bytes for Mac-side transcription", async () => {
    ...
    expect(audio).toEqual({ ok: true, bytes: wavBytes });
    expect(requests).toEqual([
  ...
  it("gets live result events with partial transcript", async () => {
    serve((req) => {
      const url = new URL(req.url);
      requests.push({
        method: req.method,
      ...
      if (url.pathname === "/result") {
        return Response.json({
          ...
          duration_ms: null,
          transcript: { text: "partial words", partial: true },
          events: [
            {
              seq: 12,
              type: "transcription",
    ...
    const result = await bridge.getResult("android-session", 10);
    ...
    expect(requests).toEqual([
      { method: "GET", path: "/result?session_id=android-session&after_seq=10" },
    ]);
  });

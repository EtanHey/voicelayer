...
export type AndroidStopResult =
  | {
      ...
      transcript?: AndroidTranscript | null;
...
export interface AndroidTranscript {
  text: string;
  partial: boolean;
  polishedText?: string | null;
}

export interface AndroidResultEvent {
  seq: number;
  type: "transcription_status" | "transcription";
  status?: string;
  message?: string;
  text?: string;
  partial?: boolean;
...
export type AndroidPullAudioResult =
...
export class AndroidApplianceBridge {
  ...
  async checkHealth(): Promise<AndroidHealthResult> {
    ...
    if (!response.ok) {
      return {
        ok: false,
        reason: response.status === 409 ? "busy" : "unhealthy",
      };
    }
  ...
  async stopRecording(sessionId: string): Promise<AndroidStopResult> {
    ...
    return {
      ...
      audioSha256:
        typeof body.audio_sha256 === "string" ? body.audio_sha256 : null,
      transcript: parseTranscript(body.transcript),
    };
  }

  async getResult(sessionId: string, afterSeq = 0): Promise<AndroidResult> {
    let response: Response;
    try {
      response = await this.fetchWithTimeout(
        `/result?session_id=${encodeURIComponent(sessionId)}&after_seq=${afterSeq}`,
        { method: "GET" },
      );
    } catch (err) {
      return { ok: false, reason: classifyFetchError(err) };
    ...
    return {
      ...
      seq: typeof body.seq === "number" ? body.seq : afterSeq,
      durationMs:
        typeof body.duration_ms === "number" ? body.duration_ms : null,
      audioAvailable: body.audio_available === true,
      audioSha256:
        typeof body.audio_sha256 === "string" ? body.audio_sha256 : null,
      transcript: parseTranscript(body.transcript),
  ...
  async pullAudio(sessionId: string): Promise<AndroidPullAudioResult> {
    let response: Response;
    try {
      response = await this.fetchWithTimeout(
...
function parseTranscript(value: unknown): AndroidTranscript | null {
  ...
  return {
    ...
    polishedText:
      typeof value.polished_text === "string"
        ? value.polished_text
...
function parseEvents(value: unknown): AndroidResultEvent[] {
  if (!Array.isArray(value)) return [];
  const events: AndroidResultEvent[] = [];
  for (const item of value) {
    if (!isObject(item) || typeof item.seq !== "number") continue;
    if (item.type === "transcription_status") {
      events.push({
        seq: item.seq,
      ...
      events.push({
        ...
        polishedText:
          typeof item.polished_text === "string"
            ? item.polished_text
            : typeof item.polishedText === "string"
              ? item.polishedText
              : null,
      });
    }

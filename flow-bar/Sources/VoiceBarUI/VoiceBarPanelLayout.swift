...
public struct VoiceBarPanelLayout: Equatable {
    ...
    public static func make(
        ...
        let notchSideRadius = max(0, Theme.notchSideRadius)
        let topFusionBandHeight = max(0, Theme.notchFusionBandHeight)
        let panelSize = CGSize(
            width: bodySize.width + (notchSideRadius * 2),
            height: bodySize.height + topFusionBandHeight
        )
        ...
        let activeHitRect = CGRect(
            x: notchSideRadius + safePadding + horizontalInset,

...
> Historical artifact restored from PR #392. Its 1,200-character `voice_ask`
> proposal is superseded by the 600-character hard maximum (with a lower
> per-request limit for short timeouts) and separate 1,200-character
> `voice_speak` limit in
> `docs/plans/2026-08-01-long-ask-guard-delivery.md`.
...
`voice_speak` acknowledgement gating is also out of scope. Rewind behavior is
unchanged and deliberately not characterized here. Changing `voice_speak`
behavior would expand this PR beyond the long-ask guard.

...
Classify the exact Shift+F5 chord before the generic key-up path. When no gesture is active, key-down remains the sole `.pasteLastTranscript` dispatch and its paired key-up is consumed without dispatch so the target does not receive an unmatched release. Track that pairing in the event-tap context so the key-up is still consumed if Shift is released before F5. When a gesture is active, Shift+F5 key-down is consumed without repasting, while key-up remains `.keyUp` so an active recording can unwind safely.
...
- Hotkey tests prove idle Shift+F5 dispatches one repaste, its later release dispatches none even when Shift is released first, active-gesture key-down cannot repaste, and active-gesture key-up still unwinds.

...
1. Write RED assertions for a seven-bar 46×24 pt non-compressible viewport, no internal clip on full-height bars, and screen-leading compact placement whose spinner/waveform delta is ≤2 pt.
...
4. Replace the hard-coded compact two-point inset with the shared 14 pt compact contract and reserve the 16 pt visible core-side fade before trailing content.
...
2. Add RED synthetic/captured-frame tests that reject transcribing completeness below 95%, recording peaks below 80% of the speaking reference, recording growth pinned off-center, unequal slot geometry, a spinner/waveform padding delta above 2 pt, and an absent/abrupt seam fade.
...
5. Post the exact artifact and scale-gated runner to the lead. The lead captures ephemerally and runs sharpness, contrast, birthmark, waveform-census, spinner-inset, and outward-fade gates; this worker does not place another surface on Etan's live notch.
...
2. Write RED assertions for one bare active-control language, centralized optical glyph metrics, and a down-right stop-glyph correction.
3. Run the focused tests and confirm the current mixed filled/bare control implementation fails.
4. Write a RED layout assertion that hover, recording, and compact-status slots use the same visible core gutter and screen-leading alignment contract.

...
> Historical execution plan restored from PR #392. Its 1,200-character
> `voice_ask` examples record the original merged design; current policy is the
> 600-character hard maximum (tightened further for short per-call timeouts)
> documented in `docs/plans/2026-08-01-long-ask-guard-delivery.md`.
...
complete report, record the deferred `voice_speak` acknowledgement gap without
characterizing rewind behavior, and ensure its final line is exactly
`TASK_DONE`.

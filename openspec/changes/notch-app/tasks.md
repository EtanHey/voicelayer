...
- [ ] 3.15 Compact gallery polish — source/TDD slice complete: idle-hover restores History + Dictionary and active flows exclude both; recording/transcribing/speaking reuse one fixed unclipped 46×24 seven-bar waveform slot; compact slots use a symmetric 13.5 pt core-facing contract; stop is a centered 8 pt square in a 20 pt destructive circle while secondary controls stay bare. Completion waits on the exact-candidate re-capture and visual grade.
...
- [ ] 3.18 Round-2D convergence — content-fit recording/transcribing wings remove the ghost tail, transient wing labels share live primary-label color, and every waveform bar owns a full-height center-aligned slot so recording→transcribing cannot pin all bottoms to one floor. Exact-head verification includes an isolated parallel-only bottom-left capture at normal window level; the lead remains the only actor allowed to swap or grade the resident notch surface.

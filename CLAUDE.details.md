...
- Transcription effort (Settings → Models, code in `src/whisper-performance.ts`): `fast` (`-bo 1 -bs 1`), `balanced` (`-bo 3 -bs 3`), `accurate` (`-bo 5 -bs 5`, default). Same `large-v3-turbo` model for all three — only whisper.cpp beam-search/best-of changes. Persisted to `~/.local/state/voicelayer/whisper-performance.json`; override per-process with `QA_VOICE_WHISPER_PERFORMANCE_EFFORT`. Changing it reloads the model (about 2 s) so the next dictation uses it.

## VoiceBar Settings (SwiftUI, `flow-bar/Sources/VoiceBarUI/SettingsView.swift`)
- Four tabs: **General · Models · Dictionary · History**. Open with ⌘, / right-click → Settings… / the notch gear, or `open voicebar://settings` (`voicebar://settings/<tab>` opens that tab, case-insensitive; an unknown tab still opens Settings; an already-open window switches via `SettingsTabRequest`).
- General: shortcut status + Check shortcut; permissions (Microphone, Accessibility, Input Monitoring) with Open links; Visibility (Hide for 1 hour / Show VoiceBar); microphone priority; Advanced (F5 key helper); last dictation.
- Models: speech model and size, in memory (Load/Unload), transcription effort, and the Processing toggles below.
- Processing toggles (`src/processing-settings.ts`): model polish, closing-phrase filter, smart chunks, smart boundaries. Persisted to `~/.local/state/voicelayer/processing-settings.json` (override: `VOICELAYER_PROCESSING_SETTINGS_PATH`); precedence is **env (when defined, even empty) > file > default**. A row whose env var is set (`QA_VOICE_STT_POLISH`, `VOICELAYER_STT_OUTRO_GATE`, `VOICELAYER_STT_SMART_CHUNKS`, `VOICELAYER_STT_SMART_BOUNDARIES`) shows "Set by <VAR>" and is locked. Every read site passes `processingEnv()`, never `process.env`.
- Dictionary: your terms + included terms, search, and the Add term sheet (same store as `voicelayer vocab`). Add to Dictionary lives here only.
...
- The QA probes (`qa_context_menu_probe`, `qa_vertical_hit_probe`, the isolated-capture receipts) are dev/CI-only (Etan ruling 2): they compile under `VOICEBAR_QA`, which debug/test builds define. A release bundle gets them only with `VOICEBAR_QA_BUILD=1 bash flow-bar/build-app.sh --install-path "$TMPDIR/VoiceBarQA.app" --no-stop --no-relaunch` (the path must end in `.app`) (needed by `scripts/verify-notch-event-handling.sh`); a notarized release refuses that, and `scripts/check-voicebar-qa-free.sh` fails any user build that still carries them (build-app.sh, release-voicebar.sh and CI run it).
...
- `VOICELAYER_STT_SMART_CHUNKS=1` opts the ≥90 s saved-WAV decode path into silence-aware chunking. **Default off** — unset, every boundary is exactly `WAV_CHUNK_SECONDS` and every seam uses the anchor merge. When on, a Silero pause map is built over the finished WAV (`src/stt-pause-map.ts`) and each chunk ends at the last pause in a 20-30 s window, else at the fixed 30 s cut. A boundary landing at least 0.5 s inside a pause is a **silence seam**: it keeps only 0.5 s of overlap instead of 5 s, its chunk texts are concatenated rather than reconciled, and the dropped-overlap witness never fires there — the two chunks share only silence, so there is no anchor to find and nothing to dedupe. If under 5 % of a recording decodes as speech the pause map is not trusted and the fixed cuts are kept. A boundary also never strands a final chunk shorter than `WAV_TAIL_VERIFY_MIN_SECONDS` (12.5 s): a recording ends in silence, so the last pause the rule can see often ends a fraction of a second before the file does, and the scrap left behind is exactly what the very-short-final-chunk check discards whole — a silence seam never has the word overlap that check reads as health. When the audio left is between one chunk and one chunk plus 12.5 s, `chooseChunkEnd` splits the remainder at its midpoint (snapped to the nearest pause keeping both sides ≥ 12.5 s) instead. Measured over the 18 recordings ≥90 s in the 2026-09-06 corpus, 6 ended on a chunk under 12.5 s, four of them under 2 s, and `2026-09-06T08-25-24` lost 29 words that way (lane C1-c). It remains opt-in. On PR #31's pre-guard schedules it was the only configuration producing **no looped text at all** (0 looped words vs 109; 0/18 adjacent duplicates vs 4/18) and ran 1.9× faster; the post-guard sweep must re-establish those properties. PR #31 also recorded it losing more content (337 words vs 210), but **that number does not hold up** — loss there was defined as "a word in the OFF decode absent from the ON decode", and OFF is the configuration that loops, so OFF's second copy of a repeated passage was booked as a word ON lost. Against a seam-free whole-file reference, sweep1 measured ON/OFF loss at 232/215 words; sweep2 measured 258/274 and reversed the sign. That instability is inside the noise of the method. See PR #31 and lane C1-c (`docs.local/c1c/FINDINGS.md`).
- `VOICELAYER_STT_SMART_BOUNDARIES=1` validates the sentence breaks in a finished transcript against where Etan actually stopped (`src/stt-sentence-boundaries.ts`). **Rule B** (his ruling, 2026-09-06): a terminal mark survives when the clause it closes is complete AND either a pause >=400 ms sits under it OR what follows starts a new subject; it is demoted to a comma only when the clause is incomplete, or when the following words carry on the same clause ("...of the next couple of words. I guess" -> "..., I guess"). Never adds a break, never drops a word, and never touches an abbreviation (`Dr.`, `e.g.`, `U.S.` — the set is shared with `src/rules-engine.ts`). Switches the whisper-server request to `response_format=verbose_json` for segment timestamps and costs one extra Silero pass over the finished WAV; that pass is abortable, so a cancelled `voice_ask` settles immediately. Segments are accepted only when their `segmentsAudioSha256` matches the WAV the pause map runs over. With an empty pause map the rule still runs on arm (ii) alone. In `QA_VOICE_STT_POLISH=shadow` the text is returned UNMODIFIED and demotions go to the shadow row only. Recordings on the >=90 s chunked path use absolute segment times assembled from each chunk. When the word-level text seam falls inside a decoder segment, the whole seam-adjacent segment is retained and joined to its predecessor rather than clamped; this widens the evidence conservatively, but C2 then distributes the kept words across that wider span and can move an internal pause-support index. At a smart-chunk silence seam, joining across the pause can also hide the very boundary C2 wants to judge. Both effects remain behind the explicit C2 flag (and silence seams additionally behind the smart-chunk flag), so C2 remains default off. Unset, the polish output remains today's.
- The deterministic terminal-punctuation floor judges `?` from the clause that actually ends the transcript, never from the first two words of a multi-sentence blob. Etan's law for this row: never upgrade `.` to `?` without an interrogative signal from that sentence. A question mark Whisper already produced is preserved, and this stage never changes internal punctuation or words.
- `VOICELAYER_STT_OUTRO_GATE` deletes a hallucinated closer — `Thank you.`, `Okay.`, `You.`, `and.`, `So, so, so, so, so.`, `תודה.`, `תודה רבה.` — from a finished transcript (`src/stt-outro-gate.ts`). **Default on:** unset, empty, and `1|true|on|yes` enable the gate; set `0|false|off|no` to disable it as an escape hatch. Unrecognised set values stay off so a typo cannot silently enable deletion. A sentence goes only when all three hold: (a) it is a complete sentence, ending in a single full stop, drawn entirely from the known-hallucination lexicon (never a fragment, never an ellipsis, never a question — two corpus rows end in an `okay?` Etan KEPT); (b) the audio region whisper attributed to it carries no *sustained* speech (60 ms over threshold), measured against that recording's OWN noise floor at +16 dB rather than a fixed dB line — the `…ac389f08` specimen has a -53.2 dBFS floor and breath windows up to -43.7 dBFS inside its silence, so the fixed -45 dB the brief proposed would have refused to gate it — **and** the span's peak is not within 6 dB of the recording's own measured speech level (median of the loudest 20 % of windows), which is what protects a low-gain recording whose speech sits close to its floor and a wall-to-wall-speech recording whose floor is measured inside speech (there is deliberately no upper clamp on the threshold); (c) that silence extends clear of the region on both sides — 0.15 s at the tail, 0.30 s for an internal phrase, since every internal `Thank you.` in the corpus is one he really said. Works at internal pauses as well as the tail. Enabling it switches whisper-server requests to `response_format=verbose_json` for segment timestamps, adding the larger response payload and JSON parsing per request (no second decode or extra Silero pass). Without segment timestamps in the response the gate is inert: the brief's "measure the last N ms of the WAV" fallback is deliberately NOT implemented, because a VAD recording stops *because* it went quiet and its last N ms are silent in every recording — a spoken "Thank you." would measure identically to an invented one. The >=90 s chunked path converts chunk-relative segments to the recording's absolute timeline; an overlap cut inside a decoder segment widens that seam span rather than clamping or interpolating it, so extra speech can veto a removal but cannot be hidden from the gate. Measured on randomized tiled segments, 4.28 of 5 seams needed this merge and seam-merged spans covered 21.7 % of the timeline; a closer inside one of those conservative spans is effectively unremovable because neighbouring speech vetoes the cut. The gate is still skipped whenever head verification, acoustic loop collapse, or echo trim rewrote the merged transcript after the segment-bearing decodes — the positional span lookup refuses stale text, so a repaired transcript keeps its closer by design. Removals are logged and add `+outro` to the backend string. See PR #33 and lane H3.

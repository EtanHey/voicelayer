...
- `VOICELAYER_STT_SEED_PROMPT=1|true|on|yes` adds a 110-character, punctuated, term-free dictation-command prompt to auto-language whisper-server requests only after capture-time speech detection succeeds. **Default off:** unset or any other value keeps today's request byte-identical. Chunked requests keep only their continuity prompt; explicit-language and no-speech requests never receive this seed.
